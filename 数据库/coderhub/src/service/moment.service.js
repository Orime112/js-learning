...
class MomentService {
  ...
  async detail(id) {
    ...
    const statement = `
      SELECT 
        ...
        ),NULL) comments FROM comment c LEFT JOIN user cu ON c.user_id = cu.id WHERE c.moment_id = m.id) comments
      FROM moment m
  ...
  async hasLabel(momentId, labelId) {
    ...
    const [result] = await connection.execute(statement, [momentId, labelId]) // ! 异步请求千万记得加 await
    return result[0];
  }
